...
def filter_products(keyword: str = None, price_min: float = 0.0, price_max: float = 999999.0):
    """
    Fetch products from Qdrant and filter by keyword/category and price range.
    """
    results, _ = qdrant.scroll(collection_name=COLLECTION_NAME, limit=200)
    ...
    for record in results:
        ...
        if not handle:  # skip products without handle
            continue
        ...
        if keyword:
            ...
            text = f"{name} {' '.join(tags)} {category}".lower()
            if keyword_lower not in text:
                detected_category = detect_category(keyword_lower)
                if detected_category and detected_category not in category:
                    continue
    ...
    # Update global cache
    global last_products_cache
    last_products_cache = {p["Handle"]: p for p in filtered}

    return filtered[:10]  # top 10 matches
...
# ===========================
@tool
def check_inventory(handle: str) -> Dict:
    """Checks availability of a previously recommended product"""

    if handle not in last_products_cache:
        return {
            "available": False,
            "message": "❌ Please select a product from the shown list"
        }
...
def checkout(handle: str) -> Dict:
    """
    Adds a previously recommended product to the cart.
    Rejects any handle not shown to the user.
    """
    ...
    p = last_products_cache[handle]

    return {
        "success": True,
        "source": "cache",          # 🔐 guarantees correctness
        "action": "add_to_cart",
                ...
                [
                    SystemMessage(
                        content="""You are a sales assistant.
                                    ...
                                    STRICT RULES:
                                    - Only use these products: {', '.join(available_names)}
                                    - Only call checkout with these handles: {', '.join(available_handles)}
                                    - NEVER invent product names or handles.
                                    - ONLY display products returned by filter_products in tabular form
                                    - ALWAYS display Product Name + Handle together
                                    - If the user asks about a product not in the last list, reply:
                                    "Please select a product from the shown list."
...
with col2:
    ...
    if not st.session_state.cart:
        ...
        st.dataframe(cart_df[["Product Name", "Handle", "Price"]], use_container_width=True, hide_index=True)
        ...
        for item in st.session_state.cart:
            ...
            with col_a:
                st.write(f"• {item['Product Name']} (${item['Price']})")
            with col_b:
                if st.button("Remove", key=f"remove_{item['Handle']}"):
                    st.session_state.cart = [x for x in st.session_state.cart if x['Handle'] != item['Handle']]
